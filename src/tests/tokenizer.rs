...
fn tokenizer_operator_test(){
    ...
    let expected = vec![Token::new(TokenType::OperatorEqual,"=".into(),0)];
    assert_eq!(equal,expected);

    let src = "+";
    let plus = Lexer::tokenize(&src).unwrap();
    let expected = vec![Token::new(TokenType::OperatorPlus,"+".into(),0)];
    assert_eq!(plus,expected);

    let src = "-";
    let minus = Lexer::tokenize(&src).unwrap();
    let expected = vec![Token::new(TokenType::OperatorMinus,"-".into(),0)];
    assert_eq!(minus,expected);

    let src = "*";
    let multi = Lexer::tokenize(&src).unwrap();
    let expected = vec![Token::new(TokenType::OperatorMultiplication,"*".into(),0)];
...
fn tokenizer_separator_test(){
    ...
    let expected = vec![Token::new(TokenType::SeparatorCurvedBracketOpen,curved_bracket_open.to_string(),0)];
    assert_eq!(expected,tokens);

    let curved_bracket_closed = "}";
    let tokens = Lexer::tokenize(&curved_bracket_closed).unwrap();
    let expected = vec![Token::new(TokenType::SeparatorCurvedBracketClosed,curved_bracket_closed.to_string(),0)];
    assert_eq!(expected,tokens);

    let bracket_open = "(";
    let tokens = Lexer::tokenize(&bracket_open).unwrap();
    let expected = vec![Token::new(TokenType::SeparatorBracketOpen,bracket_open.to_string(),0)];
    assert_eq!(expected,tokens);

    let bracket_closed = ")";
    let tokens = Lexer::tokenize(&bracket_closed).unwrap();
    let expected = vec![Token::new(TokenType::SeparatorBracketClose,bracket_closed.to_string(),0)];
    assert_eq!(expected,tokens);
...
fn tokenizer_numbers_test(){
    ...
    let expected = vec![Token::new(TokenType::LiteralFloat,float.to_string(),0)];
    assert_eq!(expected,tokens);
    ...
    let expected = Ok(vec![Token::new(TokenType::LiteralFloat,valid_float2.into(),0)]);
    assert_eq!(expected,tokens);

    let integer = "6";
    let tokens = Lexer::tokenize(&integer);
    let expected = Ok(vec![Token::new(TokenType::LiteralInteger,integer.into(),0)]);
    assert_eq!(expected,tokens);

    let identifier = "b6";
    let tokens = Lexer::tokenize(&identifier);
    let expected = Ok(vec![Token::new(TokenType::Identifier,identifier.into(),0)]);
    assert_eq!(expected,tokens);

    let nan = "6b";
    let tokens = Lexer::tokenize(&nan);
    let expected = Ok(vec![Token::new(TokenType::LiteralInteger,"6b".into(),0)]);
    assert_eq!(expected,tokens);
}
